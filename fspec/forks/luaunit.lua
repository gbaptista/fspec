    ...
    * result: if success is true, a multi-line string with deep analysis of the two lists
    ...
    local function insertABValue(ai, bi)
        ...
        if M.private.is_table_equals( table_a[ai], table_b[bi], margin) then
            return extendWithStrFmt( result, '  = A[%d], B[%d]: %s', ai, bi, M.prettystr(table_a[ai]) )
        else
            extendWithStrFmt( result, '  - A[%d]: %s', ai, M.prettystr(table_a[ai]))
            extendWithStrFmt( result, '  + B[%d]: %s', bi, M.prettystr(table_b[bi]))
        end
    end
    ...
    if shortest - commonBackTo <= longest - commonBackTo - 1 then
        ...
        for i = shortest - commonBackTo, longest - commonBackTo - 1 do
            if len_a > len_b then
                extendWithStrFmt( result, '  - A[%d]: %s', i, M.prettystr(table_a[i]) )
                -- table.insert( result, '+ (no matching B index)')
            else
                -- table.insert( result, '- no matching A index')
                extendWithStrFmt( result, '  + B[%d]: %s', i, M.prettystr(table_b[i]) )
            end
        end
    end
...
local function prettystrPairs(value1, value2, suffix_a, suffix_b)
    ...
    local str1, str2 = M.prettystr(value1), M.prettystr(value2)
...
local function _table_tostring( tbl, indentLevel, printTableRefs, cycleDetectTable )
    ...
    if mt and mt.__tostring then
        ...
        if type(result) ~= 'string' then
            return string.format( '<invalid tostring() result: "%s" >', M.prettystr(result) )
        end
        result = strsplit( '\n', result )
...
local function errorMsgEquality(actual, expected, doDeepAnalysis, margin)
    ...
    if type(expected) == 'string' or type(expected) == 'table' then
        ...
        if margin then
            result = result .. '\nwere not equal by the margin of: '..M.prettystr(margin)
        end
    ...
    return string.format("expected: %s, actual: %s",
                         M.prettystr(expected), M.prettystr(actual))
end
...
function M.runOnlyIf( cond, msg )
    ...
    if not cond then
        error_fmt(2, M.SKIP_PREFIX .. M.prettystr(msg))
    end
end
...
function M.almostEquals( actual, expected, margin )
    if type(actual) ~= 'number' or type(expected) ~= 'number' or type(margin) ~= 'number' then
        error_fmt(3, 'almostEquals: must supply only number arguments.\nArguments supplied: %s, %s, %s',
            M.prettystr(actual), M.prettystr(expected), M.prettystr(margin))
    end
    if margin < 0 then
...
function M.assertAlmostEquals( actual, expected, margin, extra_msg_or_nil )
    ...
    if type(margin) ~= 'number' then
        error_fmt(2, 'almostEquals: margin must be a number, not %s', M.prettystr(margin))
    end
...
function M.assertNotEquals(actual, expected, extra_msg_or_nil)
    ...
    fail_fmt(2, extra_msg_or_nil, 'Received the not expected value: %s', M.prettystr(actual))
end
...
function M.assertErrorMsgContains( partialMsg, func, ... )
    ...
    local no_error, error_msg = pcall( func, ... )
    if no_error then
        failure( 'No error generated when calling function but expected error containing: '..M.prettystr(partialMsg), nil, 2 )
    end
...
function M.assertEvalToTrue(value, extra_msg_or_nil)
    if not value then
        failure("expected: a value evaluating to true, actual: " ..M.prettystr(value), extra_msg_or_nil, 2)
    end
end

function M.assertEvalToFalse(value, extra_msg_or_nil)
    if value then
        failure("expected: false or nil, actual: " ..M.prettystr(value), extra_msg_or_nil, 2)
    end
end

function M.assertIsTrue(value, extra_msg_or_nil)
    if value ~= true then
        failure("expected: true, actual: " ..M.prettystr(value), extra_msg_or_nil, 2)
    end
end

function M.assertNotIsTrue(value, extra_msg_or_nil)
    if value == true then
        failure("expected: not true, actual: " ..M.prettystr(value), extra_msg_or_nil, 2)
    end
end

function M.assertIsFalse(value, extra_msg_or_nil)
    if value ~= false then
        failure("expected: false, actual: " ..M.prettystr(value), extra_msg_or_nil, 2)
    end
end

function M.assertNotIsFalse(value, extra_msg_or_nil)
    if value == false then
        failure("expected: not false, actual: " ..M.prettystr(value), extra_msg_or_nil, 2)
    end
end

function M.assertIsNil(value, extra_msg_or_nil)
    if value ~= nil then
        failure("expected: nil, actual: " ..M.prettystr(value), extra_msg_or_nil, 2)
    end
end
...
function M.assertIsNaN(value, extra_msg_or_nil)
    if type(value) ~= "number" or value == value then
        failure("expected: NaN, actual: " ..M.prettystr(value), extra_msg_or_nil, 2)
    end
end
...
function M.assertIsInf(value, extra_msg_or_nil)
    if type(value) ~= "number" or math.abs(value) ~= math.huge then
        failure("expected: #Inf, actual: " ..M.prettystr(value), extra_msg_or_nil, 2)
    end
end
...
function M.assertNotIsInf(value, extra_msg_or_nil)
    if type(value) == "number" and math.abs(value) == math.huge then
        failure("expected: not infinity, actual: " .. M.prettystr(value), extra_msg_or_nil, 2)
    end
end

function M.assertIsPlusZero(value, extra_msg_or_nil)
    if type(value) ~= 'number' or value ~= 0 then
        failure("expected: +0.0, actual: " ..M.prettystr(value), extra_msg_or_nil, 2)
...
----------------------------------------------------------------
    ...
    function M.LuaUnit:updateStatus( err )
        ...
        elseif err.status == NodeStatus.SKIP then
            ...
            error('No such status: ' .. M.prettystr(err.status))
        end
    ...
    function M.LuaUnit:endTest()
        ...
        elseif node:isSkipped() then
            ...
            error('No such node status: ' .. M.prettystr(node.status))
        end
        ...
        for i,v in ipairs( listOfNameAndInst ) do
            ...
            if M.LuaUnit.asFunction(instance) then
                ...
                if type(instance) ~= 'table' then
                    error( 'Instance must be a table or a function, not a '..type(instance)..' with value '..M.prettystr(instance))
                end
